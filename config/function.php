...
{
    // Jika string kegiatan kosong, kembalikan pesan
    if (empty($string_kegiatan)) {
    ...
    foreach ($array_kegiatan as $kgt) {
        // Periksa apakah format string valid
        $pattern = "/^.+\s*\(([\d:]+)\s*-\s*([\d:]+)\)$/";
        if (preg_match($pattern, $kgt, $matches)) {
            if (isset($matches[1]) && isset($matches[2])) {
                $waktu_awal = trim($matches[1]);
                $waktu_akhir = trim($matches[2]);

                // Validasi format waktu
                if (strtotime($waktu_awal) !== false && strtotime($waktu_akhir) !== false) {
    ...
    // Tampilkan waktu kegiatan
    if (!empty($kegiatan)) {
        foreach ($kegiatan as $waktu) {
            echo $waktu . '</br>';
        }
    } else {
        echo "Belum ada waktu kegiatan yang tercatat";
    }
}
?>
