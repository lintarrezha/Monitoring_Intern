...
<form action="apps/pengguna/mulai_absensi.php" method="post" enctype="multipart/form-data">
    ...
    <div class="row">
        <div class="col-sm-4">
            <div class="form-group">
                <br>
                <button type="submit" name="submit" id="tombol_hari" class="simpan_absensi btn btn-primary" <?php echo $absensi_sudah; ?>><i class="fa fa-clock-o"></i> Presensi</button>
            </div>
        </div>
    </div>
</form>
...
<script>
    $('.simpan_absensi').on('click',function(){
        konfirmasi=confirm("Konfirmasi sebelum Presensi?")
        if (konfirmasi){
            return true;
        }else {
            return false;
